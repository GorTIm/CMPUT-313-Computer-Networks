...
//newly added handler to forward the message 
//when a router receive it 
static EVENT_HANDLER(frame_forward){
...
{
    //determind what kind the current ndoe is 
    //and then go to diffferent process
    if (nodeinfo.nodetype == NT_ROUTER){
        CHECK(CNET_set_handler( EV_PHYSICALREADY, frame_forward   , 0));
        }
    else{
